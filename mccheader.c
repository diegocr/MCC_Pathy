...
STATIC CONST UBYTE UserLibName[] = CLASS;
...
STATIC CONST UBYTE __muimaster_name[] = MUIMASTER_NAME;
...
INLINE BOOL UserLibOpen(struct Library *base)
{
  ...
  if((MUIMasterBase = OpenLibrary(__muimaster_name, MASTERVERSION)) &&
...
#ifdef STACK
# ifndef CLASS_STACKSWAP
#  define CLASS_STACKSWAP 1
# endif
#else
# define STACK	16384
#endif
...
struct LibraryHeader * ASM SAVEDS LibInit(REG(a0, BPTR Segment), REG(a6, struct ExecBase *sb))
{
  ...
  #if defined(CLASS_STACKSWAP)
  if ( !( stack = AllocMem( sizeof( struct StackSwapStruct ) + STACK, MEMF_PUBLIC | MEMF_CLEAR ) ) )
    return( NULL );

  stack->stk_Lower  = (APTR)( (ULONG)stack + sizeof( struct StackSwapStruct ) );
  stack->stk_Upper  = (ULONG)( (ULONG)stack->stk_Lower + STACK );
  ...
  FreeMem(base->lh_Stack, sizeof(struct StackSwapStruct) + STACK);
  #endif
